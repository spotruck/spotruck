...
interface CandidatureRecente {
  ...
  note: number;
  score: number;
  statut: "EN ATTENTE" | "RETENU" | "REFUSÉ";
  ville: string;
}
...
export default function OrganisateurDashboardClient({
  ...
  return (
    <main style={{ minHeight:"100vh", backgroundColor:S.cream, color:S.brown, display:"grid", gridTemplateColumns:"260px 1fr" }}>
      <OrganisateurSidebar
        active="/dashboard/organisateur"
        userData={userData}
        badges={{ "/dashboard/organisateur/candidatures": stats.candidaturesNonTraitees }}
      />
      ...
      <div style={{ padding:"3rem", maxWidth:1100, minWidth:0 }}>
        ...
        <div style={{ marginBottom:"2.5rem" }}>
          ...
          <h1 style={{ fontFamily:S.serif, fontSize:"2.4rem", fontWeight:800, lineHeight:1.1, marginBottom:"0.3rem" }}>
            Bonjour{userData.prenom ? `, ${userData.prenom}` : ""}.
          </h1>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"2px", marginBottom:"3rem" }}>
          {[
            { label:"ÉVÉNEMENTS PUBLIÉS",  value:String(stats.evenementsPublies),  sub:`dont ${stats.evenementsActifs} actif${stats.evenementsActifs > 1 ? "s" : ""} en cours` },
            { label:"CANDIDATURES REÇUES", value:String(stats.candidaturesRecues), sub:`${stats.candidaturesNonTraitees} non traitées` },
            { label:"TRUCKS RETENUS",      value:String(stats.trucksRetenus),      sub: nomEvenementActif ? `pour ${nomEvenementActif}` : "—" },
          ].map(k => (
          ...
          <div style={{ marginBottom:"3rem", backgroundColor:"rgba(196,98,45,0.07)", border:`1px solid rgba(196,98,45,0.25)`, padding:"1.25rem 1.5rem", display:"flex", gap:"1rem", alignItems:"flex-start" }}>
            ...
            <div>
              ...
              <p style={{ fontFamily:S.sans, fontSize:"0.82rem", fontWeight:300, color:S.brown, lineHeight:1.7 }}>
                Consultez vos candidatures en attente et retenez les trucks les mieux notés pour compléter votre offre.
              </p>
            </div>
          </div>
        )}
        ...
        <div>
          <div style={{ marginBottom:"1.5rem", paddingBottom:"0.75rem", borderBottom:`1px solid ${S.border}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              ...
              {nomEvenementActif && (
                <p style={{ fontFamily:S.sans, fontSize:"0.72rem", fontWeight:300, color:S.muted, marginTop:"0.2rem" }}>{nomEvenementActif}</p>
              )}
            </div>
          ...
          {candidaturesRecentes.length === 0 ? (
            <div style={{ backgroundColor:S.card, padding:"2rem", textAlign:"center" }}>
              <p style={{ fontFamily:S.sans, fontSize:"0.82rem", fontWeight:300, color:S.muted }}>
